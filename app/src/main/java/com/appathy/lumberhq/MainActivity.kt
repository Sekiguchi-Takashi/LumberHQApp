...
class MainActivity : Activity() {
    ...
    private val depts = listOf(
        ...
        Dept("kenpin", "品質・倉庫部", "品質保管AI",
            ...
            "在庫差異ゼロ / 検品48時間以内",
            "個体カード台帳(簡易版)"),
        Dept("hanbai", "販売・物流部", "販売物流AI",
            "受注獲得から配達完了まで一気通貫。「送料込みで儲かる売り方」を商品ごとに設計し、出品前に配送手段と実質粗利を確定させる。",
            listOf("個体カードからの商品ページ生成", "送料込み値付けと価格改定ルール運用", "梱包チェックリストと出荷バッチ指示", "勝ち筋分析を調達部へ毎週還元"),
            "実質粗利 / 発送2営業日以内 / 破損率1%未満",
            "配送サイズ判定"),
        Dept("marketing", "顧客接点部", "顧客接点AI",
    ...
    private fun toolShipSize(root: LinearLayout) {
        ...
        root.addView(result)
        root.addView(text("※区分は一般的な目安。実料金は送料マスタ(サービス調達部)を正とする", 11f, sub))
    }
